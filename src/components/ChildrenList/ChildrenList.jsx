...
import { FaCommentDots, FaPlus, FaHome, FaTree, FaUserTie, FaBuilding, FaTimes, FaMapMarkerAlt, FaPen, FaCheck, FaTrash, FaClipboardList, FaCalendarPlus, FaTrashAlt, FaSpinner, FaUsers, FaList } from 'react-icons/fa';
...
const ChildrenList = () => {
    ...
    const [showTooltipActivity, setShowTooltipActivity] = useState(false); // טול-טיפ לכפתור הבית
      const [showTooltipActivityList, setShowTooltipActivityList] = useState(false); // טול-טיפ לכפתור הבית
    ...
    const handleSaveActivity = async () => {
        ...
        try {
            const response = await fetchWithAuth(`http://localhost:8000/activities/${activityId}/groups`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    useEffect(() => {
        fetchWithAuth('http://localhost:8000/children/')
        ...
        <div className="secretry-children-list-wrapper">
            ...
            <div >
                  <button className='children-button' onClick={() => setShowChildrenList(!showChildrenList)}
                    onMouseEnter={() => setShowTooltipActivityList(true)}
                    onMouseLeave={() => setShowTooltipActivityList(false)}>
                    <FaList  className="notification-icon-style" color="#3f3939" size={24}/>       </button>
            
                  {showChildrenList && navigate('/secretaryList', { state: { user_id: state?.user_id } })}
                  {showTooltipActivityList && <div className="children-tooltip">Activities List</div>}
